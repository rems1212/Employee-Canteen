...
const EmployeeSchema = new mongoose.Schema({
  ...
  canteen: { type: String }, // Added canteen field to associate employee with canteen
  salary: Number,
...
app.get('/api/employees', async (req, res) => {
  try {
    ...
    let employees;
    if (decoded.role === 'manager') {
      employees = await Employee.find({ managerId: decoded.id });
    } else {
...
// New route to get employees by canteen
app.get('/api/employees/canteen/:canteen', async (req, res) => {
  try {
    const employees = await Employee.find({ canteen: req.params.canteen });
    res.json(employees);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
